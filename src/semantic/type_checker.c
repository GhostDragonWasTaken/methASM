...
Type* type_checker_infer_type(TypeChecker* checker, ASTNode* expression) {
    ...
    switch (expression->type) {
        ...
        case AST_BINARY_EXPRESSION: {
            ...
            return type_checker_check_binary_expression(checker, binop, expression->location);
        }
        ...
        case AST_FUNCTION_CALL: {
            ...
            if (func_symbol->kind != SYMBOL_FUNCTION) {
                const char* symbol_type = (func_symbol->kind == SYMBOL_VARIABLE) ? "variable" : 
                                        (func_symbol->kind == SYMBOL_STRUCT) ? "struct" : "symbol";
                char error_msg[512];
        ...
        case AST_MEMBER_ACCESS: {
            ...
            if (object_type && object_type->kind == TYPE_STRUCT) {
                ...
                if (field_type) {
                    ...
                    char error_msg[512];
                    snprintf(error_msg, sizeof(error_msg), "Field '%s' not found in struct '%s'", 
                ...
                SourceLocation location = expression->location;
                char error_msg[512];
                snprintf(error_msg, sizeof(error_msg), "Cannot access field on non-struct type '%s'", 
                         object_type->name);
                type_checker_set_error_at_location(checker, location, error_msg);
                return NULL;
            }
...
void type_checker_init_builtin_types(TypeChecker* checker) {
    ...
    Type* point_type = type_create_struct("Point", point_field_names, point_field_types, 2);
...
int type_checker_process_struct_declaration(TypeChecker* checker, ASTNode* struct_decl) {
    ...
    if (existing) {
        type_checker_report_duplicate_declaration(checker, struct_decl->location, decl->name);
        return 0;
    }
    ...
    for (size_t i = 0; i < decl->field_count; i++) {
        ...
        if (!field_types[i]) {
            char error_msg[512];
            snprintf(error_msg, sizeof(error_msg), "Unknown type '%s' in struct field", decl->field_types[i]);
            type_checker_set_error_at_location(checker, struct_decl->location, error_msg);
            free(field_types);
            return 0;
        }
    }
    ...
    switch (declaration->type) {
        case AST_VAR_DECLARATION: {
            ...
            if (!var_decl || !var_decl->name) {
                type_checker_set_error_at_location(checker, declaration->location, "Invalid variable declaration");
                return 0;
            }
            
            // Check for duplicate declaration in current scope
            Symbol* existing = symbol_table_lookup_current_scope(checker->symbol_table, var_decl->name);
            if (existing) {
                type_checker_report_duplicate_declaration(checker, declaration->location, var_decl->name);
                return 0;
            }
            ...
            if (var_decl->initializer) {
                ...
                if (!init_type) {
                    type_checker_set_error_at_location(checker, var_decl->initializer->location,
                                                     "Cannot infer type of initializer for variable '%s'", var_decl->name);
                    return 0;
                }
                ...
                if (var_type) {
                    ...
                    if (!type_checker_is_assignable(checker, var_type, init_type)) {
                        type_checker_report_type_mismatch(checker, var_decl->initializer->location,
                ...
                type_checker_set_error_at_location(checker, declaration->location,
                                                 "Variable '%s' must have either a type annotation or an initializer", var_decl->name);
                return 0;
            }
        ...
        case AST_FUNCTION_DECLARATION: {
            ...
            if (!func_decl || !func_decl->name) {
                type_checker_set_error_at_location(checker, declaration->location, "Invalid function declaration");
                return 0;
            }
            
            // Check for duplicate declaration in current scope
            Symbol* existing = symbol_table_lookup_current_scope(checker->symbol_table, func_decl->name);
            if (existing) {
                type_checker_report_duplicate_declaration(checker, declaration->location, func_decl->name);
                return 0;
            }
            ...
            if (func_decl->parameter_count > 0) {
                ...
                for (size_t i = 0; i < func_decl->parameter_count; i++) {
                    for (size_t j = i + 1; j < func_decl->parameter_count; j++) {
                        if (strcmp(func_decl->parameter_names[i], func_decl->parameter_names[j]) == 0) {
                ...
                for (size_t i = 0; i < func_decl->parameter_count; i++) {
                    ...
                    if (!param_types[i]) {
                        type_checker_report_undefined_symbol(checker, declaration->location,
                                                           func_decl->parameter_types[i], "type");
                        free(param_types);
                        return 0;
                    }
                }
            ...
            if (func_decl->parameter_count > 0) {
                for (size_t i = 0; i < func_decl->parameter_count; i++) {
                    Symbol* param_symbol = symbol_create(func_decl->parameter_names[i], SYMBOL_VARIABLE, param_types[i]);
                    if (!param_symbol) {
                        type_checker_set_error_at_location(checker, declaration->location, "Failed to create parameter symbol");
                        symbol_table_exit_scope(checker->symbol_table);
                        return 0;
                    }
                    if (!symbol_table_declare(checker->symbol_table, param_symbol)) {
                        type_checker_report_duplicate_declaration(checker, declaration->location, func_decl->parameter_names[i]);
            ...
            if (func_decl->body) {
                for (size_t i = 0; i < func_decl->body->child_count; i++) {
                    if (!type_checker_check_statement(checker, func_decl->body->children[i])) {
        ...
        case AST_ASSIGNMENT: {
            ...
            if (!assignment || !assignment->variable_name || !assignment->value) {
                type_checker_set_error_at_location(checker, declaration->location, "Invalid assignment statement");
                return 0;
            }
            ...
            if (!var_symbol) {
                type_checker_report_undefined_symbol(checker, declaration->location,
                                                   assignment->variable_name, "variable");
                return 0;
            }
            
            if (var_symbol->kind != SYMBOL_VARIABLE && var_symbol->kind != SYMBOL_PARAMETER) {
                char error_msg[512];
                const char* symbol_type = (var_symbol->kind == SYMBOL_FUNCTION) ? "function" : 
                                        (var_symbol->kind == SYMBOL_STRUCT) ? "struct" : "symbol";
                snprintf(error_msg, sizeof(error_msg), "'%s' is a %s and cannot be assigned to", 
                        assignment->variable_name, symbol_type);
                type_checker_set_error_at_location(checker, declaration->location, error_msg);
                return 0;
            }
            ...
            if (!value_type) {
                type_checker_set_error_at_location(checker, assignment->value->location,
            ...
            if (!type_checker_is_assignable(checker, var_symbol->type, value_type)) {
                type_checker_report_type_mismatch(checker, assignment->value->location,
                                                var_symbol->type->name, value_type->name);
                return 0;
            }
...
void type_checker_set_error_at_location(TypeChecker* checker, SourceLocation location, const char* format, ...) {
    ...
    if (checker->error_reporter) {
        ...
        error_reporter_add_error(checker->error_reporter, ERROR_SEMANTIC, location, message);
    }
...
void type_checker_report_type_mismatch(TypeChecker* checker, SourceLocation location, 
    ...
    if (!checker || !expected || !actual) return;
    
    char error_msg[512];
    snprintf(error_msg, sizeof(error_msg), "Type mismatch: expected '%s', found '%s'", expected, actual);
    ...
    if (checker->error_reporter) {
        const char* suggestion = error_reporter_suggest_for_type_mismatch(expected, actual);
        if (suggestion) {
            error_reporter_add_error_with_suggestion(checker->error_reporter, ERROR_TYPE, 
                                                   location, error_msg, suggestion);
        } else {
            error_reporter_add_error(checker->error_reporter, ERROR_TYPE, location, error_msg);
        }
    }
}
...
void type_checker_report_undefined_symbol(TypeChecker* checker, SourceLocation location, 
    ...
    if (!checker || !symbol_name || !symbol_type) return;
    
    char error_msg[512];
    snprintf(error_msg, sizeof(error_msg), "Undefined %s '%s'", symbol_type, symbol_name);
    ...
    if (checker->error_reporter) {
        char suggestion[256];
        snprintf(suggestion, sizeof(suggestion), "declare '%s' before using it", symbol_name);
        error_reporter_add_error_with_suggestion(checker->error_reporter, ERROR_SEMANTIC, 
                                               location, error_msg, suggestion);
    }
}
...
void type_checker_report_duplicate_declaration(TypeChecker* checker, SourceLocation location, 
    ...
    if (!checker || !symbol_name) return;
    
    char error_msg[512];
    snprintf(error_msg, sizeof(error_msg), "Duplicate declaration of '%s'", symbol_name);
    ...
    if (checker->error_reporter) {
        char suggestion[256];
        snprintf(suggestion, sizeof(suggestion), "use a different name or remove the duplicate declaration");
        error_reporter_add_error_with_suggestion(checker->error_reporter, ERROR_SEMANTIC, 
                                               location, error_msg, suggestion);
    }
}
...
void type_checker_report_scope_violation(TypeChecker* checker, SourceLocation location,
    ...
    if (!checker || !symbol_name || !violation_type) return;
    
    char error_msg[512];
    snprintf(error_msg, sizeof(error_msg), "Scope violation: %s '%s'", violation_type, symbol_name);
    ...
    if (checker->error_reporter) {
        char suggestion[256];
        if (strcmp(violation_type, "cannot access local variable from outer scope") == 0) {
            snprintf(suggestion, sizeof(suggestion), "declare '%s' in the current scope or pass it as a parameter", symbol_name);
            ...
            snprintf(suggestion, sizeof(suggestion), "check the scope rules for '%s'", symbol_name);
        }
        error_reporter_add_error_with_suggestion(checker->error_reporter, ERROR_SCOPE, 
                                               location, error_msg, suggestion);
    }
}
...
int type_checker_check_statement(TypeChecker* checker, ASTNode* statement) {
    ...
    switch (statement->type) {
            ...
            if (ret_stmt && ret_stmt->value) {
                ...
                if (!value_type) {
                    ...
                    if (!checker->has_error) {
                        type_checker_set_error_at_location(checker, ret_stmt->value->location,
                ...
                if (checker->current_function) {
                    Type* func_return_type = checker->current_function->data.function.return_type;
                    if (!type_checker_is_assignable(checker, func_return_type, value_type)) {
                        type_checker_report_type_mismatch(checker, ret_stmt->value->location,
                                                        func_return_type->name, value_type->name);
                        return 0;
                    }
                } else {
        ...
        case AST_IF_STATEMENT: {
            ...
            // Check then branch
            if (if_stmt->then_branch && !type_checker_check_statement(checker, if_stmt->then_branch)) {
                return 0;
            }
            
            // Check else branch if present
            if (if_stmt->else_branch && !type_checker_check_statement(checker, if_stmt->else_branch)) {
                return 0;
            }
        ...
        case AST_WHILE_STATEMENT: {
            ...
            // Check condition type
            Type* condition_type = type_checker_infer_type(checker, while_stmt->condition);
            if (!condition_type) {
                return 0; // Error already reported
            }
            ...
            // Condition should be a numeric type (treated as boolean)
            if (!type_checker_is_numeric_type(condition_type)) {
                type_checker_report_type_mismatch(checker, while_stmt->condition->location,
                                                "numeric type", condition_type->name);
                return 0;
            }
            
            // Check body
            if (while_stmt->body && !type_checker_check_statement(checker, while_stmt->body)) {
                return 0;
            }
...
int type_checker_check_expression(TypeChecker* checker, ASTNode* expression) {
    if (!checker || !expression) return 0;
...
Type* type_checker_check_binary_expression(TypeChecker* checker, BinaryExpression* binop, SourceLocation location) {
    ...
    if (strcmp(op, "==") == 0 || strcmp(op, "!=") == 0 ||
        ...
        if (!type_checker_are_compatible(left_type, right_type)) {
            char error_msg[512];
            snprintf(error_msg, sizeof(error_msg), 
                    "Cannot compare '%s' with '%s'", left_type->name, right_type->name);
            type_checker_set_error_at_location(checker, location, error_msg);
            return NULL;
        }
