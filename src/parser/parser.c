...
Parser* parser_create_with_error_reporter(Lexer* lexer, ErrorReporter* error_reporter) {
...
int parser_expect_statement_end(Parser* parser) {
    ...
    parser_set_error(parser, "Expected ';' or newline at the end of the statement");
    return 0;
}
...
int parser_expect(Parser* parser, TokenType type) {
    ...
    snprintf(error_msg, sizeof(error_msg), "Expected %s, found %s", expected_str, actual_str);
    
    // Generate context-specific suggestions
    const char* suggestion = NULL;
    if (type == TOKEN_SEMICOLON) {
        suggestion = "add a semicolon ';' to end the statement";
    } else if (type == TOKEN_RPAREN && parser->current_token.type == TOKEN_COMMA) {
    ...
    } else if (type == TOKEN_COLON && parser->current_token.type == TOKEN_EQUALS) {
...
void parser_set_error_with_suggestion(Parser* parser, const char* message, const char* suggestion) {
    ...
    if (parser->error_reporter) {
        ...
        if (suggestion) {
            error_reporter_add_error_with_suggestion(parser->error_reporter, ERROR_SYNTAX, 
                                                   location, message, suggestion);
        } else {
            // Try to generate a helpful suggestion
            const char* auto_suggestion = NULL;
            if (parser->current_token.value) {
                auto_suggestion = error_reporter_suggest_for_token(parser->current_token.value);
            }
            
            if (auto_suggestion) {
                error_reporter_add_error_with_suggestion(parser->error_reporter, ERROR_SYNTAX,
                                                       location, message, auto_suggestion);
            } else {
                error_reporter_add_error(parser->error_reporter, ERROR_SYNTAX, location, message);
            }
        }
    }
}
...
void parser_synchronize(Parser* parser) {
    ...
    while (parser->current_token.type != TOKEN_EOF) {
        if (parser->current_token.type == TOKEN_SEMICOLON || parser->current_token.type == TOKEN_NEWLINE) {
            parser_advance(parser); 
            return; 
        }
    ...
    switch (parser->current_token.type) {
        ...
        case TOKEN_IDENTIFIER:
            ...
            if (parser->peek_token.type == TOKEN_EQUALS) {
                ...
                return parser_parse_expression(parser); // Could be function call or other expression
            }
        case TOKEN_LBRACE:
...
ASTNode* parser_parse_inline_asm(Parser* parser) {
    ...
    while (parser->current_token.type != TOKEN_RBRACE && 
        ...
        if (parser->current_token.value) {
            ...
            if (code_length + token_len + 2 >= buffer_size) { // +2 for separator and null terminator
                buffer_size *= 2;
            ...
            if (code_length > 0) {
                if (last_token_line != -1 && parser->current_token.line > last_token_line) {
...
ASTNode* parser_parse_return_statement(Parser* parser) {
    ...
    SourceLocation location = {parser->current_token.line, parser->current_token.column};parser_advance(parser); // consume 'return'
