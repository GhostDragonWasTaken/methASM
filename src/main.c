...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        } else if (strcmp(argv[i], "-g") == 0 || strcmp(argv[i], "--debug-symbols") == 0) {
            options.generate_debug_symbols = 1;
        } else if (strcmp(argv[i], "-l") == 0 || strcmp(argv[i], "--line-mapping") == 0) {
            options.generate_line_mapping = 1;
        } else if (strcmp(argv[i], "-s") == 0 || strcmp(argv[i], "--stack-trace") == 0) {
...
int compile_file(const char* input_filename, const char* output_filename, CompilerOptions* options) {
    ...
    if (options->debug_mode || options->generate_debug_symbols || 
        ...
        code_generator = code_generator_create_with_debug(symbol_table, type_checker, register_allocator, debug_info);
    } else {
        code_generator = code_generator_create(symbol_table, type_checker, register_allocator);
    }
...
cleanup:
    ...
    lexer_destroy(lexer);
    if (debug_info) debug_info_destroy(debug_info);
    free(source);
...
void print_usage(const char* program_name) {
    ...
    printf("  --debug-format <fmt> Debug format: dwarf, stabs, or map (default: dwarf)\n");
