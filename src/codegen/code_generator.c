...
CodeGenerator* code_generator_create_with_debug(SymbolTable* symbol_table, TypeChecker* type_checker, RegisterAllocator* allocator, 
    ...
    CodeGenerator* generator = code_generator_create(symbol_table, type_checker, allocator);
    if (!generator) return NULL;
...
void code_generator_register_function_parameters(CodeGenerator* generator, FunctionDeclaration* func_data) {
    ...
    int int_param_reg_index = 0;
    int float_param_reg_index = 0;
    ...
    for (size_t i = 0; i < func_data->parameter_count; i++) {
        ...
        ASTNode* param_var_decl = ast_create_var_declaration(param_name, type_name, NULL, (SourceLocation){0,0});
        if (!param_var_decl) continue;

        Symbol* param_symbol = symbol_table_lookup(generator->symbol_table, param_name);
        if (!param_symbol) {
            // This should have been created during semantic analysis, but as a fallback:
...
void code_generator_generate_function(CodeGenerator* generator, ASTNode* function) {
    ...
    if (generator->generate_debug_info) {
        code_generator_add_debug_symbol(generator, func_data->name, DEBUG_SYMBOL_FUNCTION,
                                       func_data->return_type, function->location.line, function->location.column);
        
        // Add line mapping for function start
        code_generator_add_line_mapping(generator, function->location.line, function->location.column, 
        ...
        if (body_prog) {
            for (size_t i = 0; i < body_prog->declaration_count; i++) {
                ...
                if (stmt && stmt->type == AST_VAR_DECLARATION) {
                    ...
                    if (var_decl) {
                        int var_size = code_generator_calculate_variable_size(generator, var_decl->type_name);
                        stack_size += var_size;
                        
                        // Register symbol without generating code yet
                        Symbol* var_symbol = symbol_create(var_decl->name, SYMBOL_VARIABLE, 
                                                           type_checker_get_type_by_name(generator->type_checker, var_decl->type_name));
                        if (var_symbol) {
...
void code_generator_generate_statement(CodeGenerator* generator, ASTNode* statement) {
    ...
    if (generator->generate_debug_info && statement->location.line > 0) {
        code_generator_add_line_mapping(generator, statement->location.line, statement->location.column,
                                       generator->debug_info->source_filename);
        ...
        case AST_NUMBER_LITERAL:
            {
                ...
                if (num_data) {
                    if (num_data->is_float) {
                        ...
                        if (float_label) {
                            code_generator_emit(generator, "    movsd xmm0, [%s + rip]  ; Load float from memory\n", float_label);
...
void code_generator_emit(CodeGenerator* generator, const char* format, ...) {
    ...
    if (written > 0) {
        ...
        if (generator->generate_debug_info) {
            const char* text = generator->output_buffer + generator->buffer_size - written;
...
void code_generator_function_prologue(CodeGenerator* generator, const char* function_name, int stack_size) {
    ...
    // This is a simplified approach - in practice, we'd only save registers we actually use
    if (conv_spec && conv_spec->callee_saved_registers) {
        ...
        for (size_t i = 0; i < conv_spec->callee_saved_count; i++) {
            ...
            if (reg_name && reg != REG_RBP && reg != REG_RSP) {
                ...
                if (reg == REG_RBX || reg == REG_R12 || reg == REG_R13 || reg == REG_R14 || reg == REG_R15) {
                        code_generator_emit(generator, "    push %s         ; Save callee-saved register\n", reg_name);
    ...
    if (aligned_stack_size > 0) {
        ...
        code_generator_emit(generator, "    sub $%d, %rsp    ; Allocate %d bytes on stack (aligned)\n", 
    ...
    if (aligned_stack_size > 0) {
        code_generator_emit(generator, "    ; Zero-initialize local variable space\n");
        code_generator_emit(generator, "    mov %rsp, %rdi  ; Destination for memset\n");
        code_generator_emit(generator, "    mov $0, %rax     ; Value to set (zero)\n");
        code_generator_emit(generator, "    mov $%d, %rcx    ; Number of bytes\n", aligned_stack_size);
        code_generator_emit(generator, "    rep stosb         ; Zero-fill the stack space\n");
    }
}
    ...
    if (conv_spec && conv_spec->callee_saved_registers) {
        ...
        for (int i = (int)conv_spec->callee_saved_count - 1; i >= 0; i--) {
            ...
            if (reg_name && reg != REG_RBP && reg != REG_RSP) {
                ...
                if (reg == REG_RBX || reg == REG_R12 || reg == REG_R13 || reg == REG_R14 || reg == REG_R15) {
                        code_generator_emit(generator, "    pop %s          ; Restore callee-saved register\n", reg_name);
                }
            }
        }
    }
...
void code_generator_generate_global_variable(CodeGenerator* generator, ASTNode* var_declaration) {
    ...
    if (generator->generate_debug_info) {
        code_generator_add_debug_symbol(generator, var_data->name, DEBUG_SYMBOL_VARIABLE,
                                       var_data->type_name, var_declaration->location.line, var_declaration->location.column);
    }
        ...
        if (var_data->initializer->type == AST_NUMBER_LITERAL) {
            ...
            if (num) {
                if (num->is_float) {
                    ...
                    if (var_data->type_name && strcmp(var_data->type_name, "float32") == 0) {
                    ...
                    if (var_size == 1) {
                        if (value < -128 || value > 255) {
                            emit_to_global_buffer(generator, "    ; Warning: Value %lld truncated to fit in %d bytes\n", value, var_size);
                        }
            ...
            if (str && str->value) {
                ...
                if (var_data->type_name && strcmp(var_data->type_name, "string") == 0) {
                    ...
                    if (str_label) {
                        emit_to_global_buffer(generator, "    dq %s  ; Pointer to string data\n", str_label);
                        emit_to_global_buffer(generator, "%s:\n", str_label);
            ...
            emit_to_global_buffer(generator, "    resb %d  ; Initialize from variable at runtime\n", var_size);
            emit_to_global_buffer(generator, "    ; Runtime initialization needed for variable reference\n");
        } else {
            // For complex initializers (expressions, function calls, etc.), reserve space and initialize at runtime
             emit_to_global_buffer(generator, "    resb %d  ; Complex initializer - runtime initialization\n", var_size);
         }
     } else {
...
void code_generator_generate_local_variable(CodeGenerator* generator, ASTNode* var_declaration) {
    ...
    if (symbol && symbol->kind == SYMBOL_VARIABLE) {
        ...
        if (generator->generate_debug_info) {
            debug_info_set_symbol_stack_offset(generator->debug_info, var_data->name, -offset);
        }
    }
    ...
    if (symbol->data.variable.is_in_register) {
        ...
        if (reg_name) {
            ...
            if (reg >= REG_XMM0 && reg <= REG_XMM15) {
                code_generator_emit(generator, "    movsd xmm0, %s    ; Store float to register\n", reg_name);
            } else {
                code_generator_emit(generator, "    mov %s, rax      ; Store to register\n", reg_name);
            }
        } else {
        ...
        if (symbol->type) {
            if (symbol->type->kind >= TYPE_FLOAT32 && symbol->type->kind <= TYPE_FLOAT64) {
                // Floating-point types - use XMM registers
                if (symbol->type->size == 4) {
                    code_generator_emit(generator, "    movss %xmm0, %d(%rbp)  ; Store float32\n", offset);
                } else {
                    code_generator_emit(generator, "    movsd %xmm0, %d(%rbp)  ; Store float64\n", offset);
                }
            } else if (symbol->type->kind == TYPE_STRING) {
                // String type - store pointer
                code_generator_emit(generator, "    mov %rax, %d(%rbp)     ; Store string pointer\n", offset);
            } else {
                // Integer types - use appropriate register size
                if (symbol->type->size == 1) {
                    code_generator_emit(generator, "    mov %al, %d(%rbp)      ; Store int8\n", offset);
                } else if (symbol->type->size == 2) {
                    code_generator_emit(generator, "    mov %ax, %d(%rbp)      ; Store int16\n", offset);
                } else if (symbol->type->size == 4) {
                    code_generator_emit(generator, "    mov %eax, %d(%rbp)     ; Store int32\n", offset);
                } else {
                    code_generator_emit(generator, "    mov %rax, %d(%rbp)     ; Store int64\n", offset);
                }
            }
        } else {
            // Fallback for unknown types
            code_generator_emit(generator, "    mov %rax, %d(%rbp)     ; Store (unknown type)\n", offset);
        }
    }
}
...
void code_generator_generate_variable_zero_initialization(CodeGenerator* generator, Symbol* symbol) {
    ...
    if (symbol->data.variable.is_in_register) {
        ...
        if (reg_name) {
            if (reg >= REG_XMM0 && reg <= REG_XMM15) {
                code_generator_emit(generator, "    xorps %s, %s      ; Zero float register\n", reg_name, reg_name);
            } else {
                code_generator_emit(generator, "    xor %s, %s        ; Zero integer register\n", reg_name, reg_name);
            }
        }
    } else {
        ...
        if (symbol->type) {
            if (symbol->type->kind >= TYPE_FLOAT32 && symbol->type->kind <= TYPE_FLOAT64) {
                ...
                if (symbol->type->size == 4) {
                    code_generator_emit(generator, "    movl $0, %d(%rbp)       ; Zero float32\n", offset);
                } else {
                    code_generator_emit(generator, "    movq $0, %d(%rbp)       ; Zero float64\n", offset);
                }
            } else {
                // Zero integer or pointer variable
                if (symbol->type->size == 1) {
                    code_generator_emit(generator, "    movb $0, %d(%rbp)       ; Zero int8\n", offset);
                } else if (symbol->type->size == 2) {
                    code_generator_emit(generator, "    movw $0, %d(%rbp)       ; Zero int16\n", offset);
                } else if (symbol->type->size == 4) {
                    code_generator_emit(generator, "    movl $0, %d(%rbp)       ; Zero int32\n", offset);
                } else {
                    code_generator_emit(generator, "    movq $0, %d(%rbp)       ; Zero int64/pointer\n", offset);
                }
            }
        } else {
            // Default zero initialization
            code_generator_emit(generator, "    movq $0, %d(%rbp)       ; Zero (unknown type)\n", offset);
        }
    }
}
...
int code_generator_calculate_variable_size(CodeGenerator* generator, const char* type_name) {
    ...
    if (generator && generator->symbol_table) {
        Symbol* type_symbol = symbol_table_lookup(generator->symbol_table, type_name);
        if (type_symbol && type_symbol->kind == SYMBOL_STRUCT && type_symbol->type) {
...
int code_generator_allocate_stack_space(CodeGenerator* generator, int size, int alignment) {
    ...
    if ((total_stack_used % 16) != 0) {
        ...
        code_generator_emit(generator, "    ; Added %d bytes padding for 16-byte alignment\n", padding);
    }
...
void code_generator_generate_function_call(CodeGenerator* generator, ASTNode* call_expression) {
    ...
    Symbol* func_symbol = symbol_table_lookup(generator->symbol_table, call_data->function_name);
    Type* return_type = NULL;
    if (func_symbol && func_symbol->kind == SYMBOL_FUNCTION && func_symbol->type) {
    ...
    code_generator_cleanup_stack_after_call(generator, call_data->arguments, call_data->argument_count);
    ...
    } else if (return_type && return_type->name && strcmp(return_type->name, "void") != 0) {
        code_generator_emit(generator, "    ; Return value in %rax (integer/pointer)\n");
    } else {
...
void code_generator_generate_parameter_passing(CodeGenerator* generator, ASTNode** arguments, size_t argument_count) {
    ...
    for (size_t i = 0; i < argument_count; i++) {
        if (arguments[i]) {
            ...
            Type* param_type = code_generator_infer_expression_type(generator, arguments[i]);
            int is_float = code_generator_is_floating_point_type(param_type);
    ...
    if (stack_param_count > 0) {
        ...
        for (int i = (int)argument_count - 1; i >= 0; i--) {
            if (arguments[i]) {
                ...
                if (goes_on_stack) {
                    code_generator_generate_parameter(generator, arguments[i], i, param_type);
                }
...
void code_generator_generate_parameter(CodeGenerator* generator, ASTNode* argument, int param_index, Type* param_type) {
    ...
    if (is_float) {
        ...
        if (float_reg_index < (int)conv_spec->float_param_count) {
            ...
            if (reg_name) {
                code_generator_emit(generator, "    movsd xmm0, %s    ; Float parameter %d in register\n", 
                                   reg_name, param_index + 1);
            }
            float_reg_index++;
        } else {
            // Parameter goes on stack
            code_generator_emit(generator, "    sub $8, %rsp        ; Allocate stack space for float\n");
            code_generator_emit(generator, "    movsd %xmm0, (%rsp) ; Float parameter %d on stack\n", 
                               param_index + 1);
        }
    } else {
        ...
        if (int_reg_index < (int)conv_spec->int_param_count) {
            ...
            if (reg_name) {
                ...
                if (param_type && param_type->size == 4) {
                    ...
                    src_reg = "eax";
                    if (strcmp(reg_name, "rdi") == 0) dst_reg = "edi";
                    else if (strcmp(reg_name, "rsi") == 0) dst_reg = "esi";
                    else if (strcmp(reg_name, "rdx") == 0) dst_reg = "edx";
                    else if (strcmp(reg_name, "rcx") == 0) dst_reg = "ecx";
                    else if (strcmp(reg_name, "r8") == 0) dst_reg = "r8d";
                    else if (strcmp(reg_name, "r9") == 0) dst_reg = "r9d";
                }
                
                code_generator_emit(generator, "    mov %s, %s    ; Integer parameter %d in register\n", 
            ...
            if (param_type && param_type->size <= 4) {
                ...
                code_generator_emit(generator, "    push %rax          ; Integer parameter %d on stack\n", 
                                   param_index + 1);
            } else {
                // 64-bit parameter
                code_generator_emit(generator, "    push %rax          ; Integer parameter %d on stack\n", 
...
void code_generator_save_caller_saved_registers_selective(CodeGenerator* generator) {
    ...
    code_generator_emit(generator, "    ; Save caller-saved registers (selective)\n");
    ...
    for (int i = float_param_regs; i < 8; i++) {
        if (i >= 4 || conv_spec->convention == CALLING_CONV_SYSV) {
            ...
            code_generator_emit(generator, "    movdqu %xmm%d, (%rsp)  ; Save XMM%d\n", i, i);
        }
    }
}
...
void code_generator_restore_caller_saved_registers_selective(CodeGenerator* generator) {
    ...
    code_generator_emit(generator, "    ; Restore caller-saved registers (selective)\n");
    
    // Restore XMM registers in reverse order
    int float_param_regs = (int)conv_spec->float_param_count;
    for (int i = 7; i >= float_param_regs; i--) {
        if (i >= 4 || conv_spec->convention == CALLING_CONV_SYSV) {
            code_generator_emit(generator, "    movdqu (%rsp), %xmm%d  ; Restore XMM%d\n", i, i);
...
void code_generator_cleanup_stack_after_call(CodeGenerator* generator, ASTNode** arguments, size_t argument_count) {
    ...
    if (stack_bytes > 0) {
        code_generator_emit(generator, "    add $%d, %rsp    ; Clean up %d bytes of stack parameters\n", 
                           stack_bytes, stack_bytes);
    }
    
    // Clean up shadow space for Microsoft x64
    if (conv_spec->convention == CALLING_CONV_MS_X64 && conv_spec->shadow_space_size > 0) {
        code_generator_emit(generator, "    add $%d, %rsp      ; Clean up shadow space\n", 
...
void code_generator_handle_return_value(CodeGenerator* generator, Type* return_type) {
    ...
    if (!return_type || !return_type->name) {
        code_generator_emit(generator, "    ; Unknown return type - assuming integer\n");
        return;
    }
    ...
    if (code_generator_is_floating_point_type(return_type)) {
        ...
        code_generator_emit(generator, "    ; Integer/pointer return value in rax\n");
        // Handle different integer sizes if needed
        if (return_type->size == 1) {
            code_generator_emit(generator, "    movzx eax, al    ; Zero-extend 8-bit return value\n");
        } else if (return_type->size == 2) {
            code_generator_emit(generator, "    movzx eax, ax    ; Zero-extend 16-bit return value\n");
        } else if (return_type->size == 4) {
            code_generator_emit(generator, "    ; 32-bit return value already in eax\n");
        }
...
void code_generator_align_stack_for_call(CodeGenerator* generator, int param_count) {
    ...
    if (conv_spec->convention == CALLING_CONV_MS_X64 && conv_spec->shadow_space_size > 0) {
        code_generator_emit(generator, "    sub rsp, %d      ; Allocate shadow space\n", 
                           conv_spec->shadow_space_size);
    }
    
    // If total pushes results in misaligned stack, add padding
    if ((total_pushes % 2) != 0) {
        code_generator_emit(generator, "    sub rsp, 8       ; Align stack to 16 bytes\n");
    }
}
...
Type* code_generator_infer_expression_type(CodeGenerator* generator, ASTNode* expression) {
    ...
    static Type int_type = { .kind = TYPE_INT32, .name = "int32", .size = 4, .alignment = 4 };
    static Type float_type = { .kind = TYPE_FLOAT64, .name = "float64", .size = 8, .alignment = 8 };
    static Type string_type = { .kind = TYPE_STRING, .name = "string", .size = 8, .alignment = 8 }; // pointer size
    ...
    switch (expression->type) {
        ...
        case AST_FUNCTION_CALL: {
            ...
            if (call && call->function_name) {
                Symbol* func_symbol = symbol_table_lookup(generator->symbol_table, call->function_name);
                if (func_symbol && func_symbol->kind == SYMBOL_FUNCTION && func_symbol->type) {
...
void code_generator_generate_binary_operation(CodeGenerator* generator, ASTNode* left, const char* op, ASTNode* right) {
    ...
    if (is_float) {
        ...
        code_generator_emit(generator, "    sub rsp, 8          ; Make space for float on stack\n");
        code_generator_emit(generator, "    movsd [rsp], xmm0  ; Save left operand (float)\n");

        // Generate right operand (float)
        code_generator_generate_expression(generator, right); // result in xmm0
        
        // Pop left operand into xmm1
        code_generator_emit(generator, "    movsd xmm1, [rsp]  ; Restore left operand (float)\n");
        code_generator_emit(generator, "    add rsp, 8          ; Clean up stack\n");
        ...
        if (instruction) {
            ...
            code_generator_emit(generator, "    movsd xmm0, xmm1   ; Move result to xmm0\n");
        } else {
            // Handle float comparisons
             if (strcmp(op, "==") == 0 || strcmp(op, "!=") == 0 || strcmp(op, "<") == 0 || 
                strcmp(op, "<=") == 0 || strcmp(op, ">") == 0 || strcmp(op, ">=") == 0) {
                code_generator_emit(generator, "    ucomisd xmm1, xmm0 ; Compare floats\n");
                const char* set_instruction = "nop";
                if (strcmp(op, "==") == 0) set_instruction = "sete";
                else if (strcmp(op, "!=") == 0) set_instruction = "setne";
                else if (strcmp(op, "<") == 0) set_instruction = "setb";
                else if (strcmp(op, "<=") == 0) set_instruction = "setbe";
                else if (strcmp(op, ">") == 0) set_instruction = "seta";
                else if (strcmp(op, ">=") == 0) set_instruction = "setae";
                
                code_generator_emit(generator, "    %s al             ; Set AL on condition\n", set_instruction);
                code_generator_emit(generator, "    movzx rax, al     ; Zero-extend AL to RAX\n");
            } else {
        ...
        code_generator_emit(generator, "    mov rbx, rax     ; Move right operand to RBX\n");
        ...
        if (instruction) {
            if (strcmp(op, "/") == 0) {
                ...
                code_generator_emit(generator, "    cqo                  ; Sign-extend RAX to RDX:RAX\n");
                code_generator_emit(generator, "    idiv rbx           ; Divide RDX:RAX by RBX\n");
                code_generator_emit(generator, "    mov rax, rdx     ; Move remainder to RAX\n");
            } else {
            ...
            if (strcmp(set_instruction, "nop") != 0) {
                ...
                code_generator_emit(generator, "    setne al           ; Set AL if result is not zero\n");
                code_generator_emit(generator, "    movzx rax, al    ; Zero-extend AL to RAX\n");
            } else if (strcmp(op, "||") == 0) {
    ...
    if (symbol && symbol->kind == SYMBOL_VARIABLE) {
        if (symbol->data.variable.is_in_register) {
            ...
            if (symbol->scope && symbol->scope->type == SCOPE_GLOBAL) {
                code_generator_emit(generator, "    mov rax, [%s + rip]  ; From global memory\n", variable_name);
            } else {
                // Local variable or parameter on stack
                int offset = symbol->data.variable.memory_offset;
                code_generator_emit(generator, "    mov rax, [rbp - %d]  ; From stack [rbp - %d]\n", offset, offset);
            }
        }
    } else {
...
void code_generator_store_variable(CodeGenerator* generator, const char* variable_name, const char* source_reg) {
    ...
    if (symbol && symbol->kind == SYMBOL_VARIABLE) {
        if (symbol->data.variable.is_in_register) {
            ...
            if (symbol->scope && symbol->scope->type == SCOPE_GLOBAL) {
                code_generator_emit(generator, "    mov [%s + rip], %s  ; To global memory\n", variable_name, source_reg);
            } else {
                // Local variable or parameter on stack
                int offset = symbol->data.variable.memory_offset;
                code_generator_emit(generator, "    mov [rbp - %d], %s  ; To stack [rbp - %d]\n", offset, source_reg, offset);
            }
        }
    } else {
...
void code_generator_load_string_literal(CodeGenerator* generator, const char* string_value) {
    ...
    if (label) {
        ...
        code_generator_emit(generator, "    lea rax, [%s + rip]  ; Load string address\n", label);
...
const char* code_generator_get_arithmetic_instruction(const char* op, int is_float) {
    ...
    if (is_float) {
        ...
        if (strcmp(op, "+") == 0) return "addsd";
        if (strcmp(op, "-") == 0) return "subsd";
        if (strcmp(op, "*") == 0) return "mulsd";
        if (strcmp(op, "/") == 0) return "divsd";
        return NULL;
    } else {
...
void code_generator_add_debug_symbol(CodeGenerator* generator, const char* name, 
    ...
    debug_info_add_symbol(generator->debug_info, name, type, type_name, line, column);
}
