...
ErrorReporter* error_reporter_create(const char* filename, const char* source_code) {
...
static int error_reporter_expand_capacity(ErrorReporter* reporter) {
    if (reporter->count >= reporter->capacity) {
        ...
        ErrorReport* new_errors = realloc(reporter->errors, sizeof(ErrorReport) * new_capacity);
...
void error_reporter_add_error(ErrorReporter* reporter, ErrorType type, 
    ...
    error_reporter_add_error_with_suggestion(reporter, type, location, message, NULL);
}

void error_reporter_add_error_with_suggestion(ErrorReporter* reporter, ErrorType type,
                                            SourceLocation location, const char* message, 
                                            const char* suggestion) {
    if (!reporter || reporter->count >= reporter->max_errors) return;
    
    if (!error_reporter_expand_capacity(reporter)) return;
    ...
    error->code_snippet = error_reporter_get_line_from_source(reporter->source_code, location.line);
...
void error_reporter_print_error(ErrorReporter* reporter, const ErrorReport* error) {
    ...
    // Print error header
    printf("%s%s%s: %s\n", severity_color, severity_text, ANSI_COLOR_RESET, error->message);
    ...
    if (error->code_snippet) {
        ...
        char* caret_line = error_reporter_create_caret_line(error->location.column, 1);
        if (caret_line) {
            printf("   | %s\n", caret_line);
            free(caret_line);
        }
    ...
    if (error->suggestion) {
        printf("   = %shelp%s: %s\n", ANSI_COLOR_BLUE, ANSI_COLOR_RESET, error->suggestion);
    }
}
...
char* error_reporter_get_line_from_source(const char* source, size_t line_number) {
...
const char* error_reporter_suggest_for_token(const char* token) {
    ...
    if (strcmp(token, "fucntion") == 0) return "did you mean 'function'?";
    if (strcmp(token, "retrun") == 0) return "did you mean 'return'?";
    if (strcmp(token, "strcut") == 0) return "did you mean 'struct'?";
    if (strcmp(token, "methdo") == 0) return "did you mean 'method'?";
    if (strcmp(token, "whiel") == 0) return "did you mean 'while'?";
    if (strcmp(token, "fi") == 0) return "did you mean 'if'?";
    if (strcmp(token, "itn32") == 0) return "did you mean 'int32'?";
    if (strcmp(token, "int23") == 0) return "did you mean 'int32'?";
    if (strcmp(token, "stirng") == 0) return "did you mean 'string'?";
    if (strcmp(token, "flaot32") == 0) return "did you mean 'float32'?";
    if (strcmp(token, "flaot64") == 0) return "did you mean 'float64'?";
    
    return NULL;
}

const char* error_reporter_suggest_for_type_mismatch(const char* expected, const char* actual) {
    ...
    if ((strcmp(expected, "int32") == 0 && strcmp(actual, "int64") == 0) ||
        ...
        snprintf(suggestion, sizeof(suggestion), "consider explicit casting: (%s)value", expected);
        return suggestion;
    }
